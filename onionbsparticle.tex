...
\caption{A BSP example for a simple world with two rooms, connected by a corridor: (a) shows a top-down view of the world, where the arrows represent the facings of the world polygons and the dashed lines represent split planes in the BSP tree; (b) shows a possible BSP tree that might be constructed for such a world; (c) shows what the world looks like in \emph{hesperus}, with portals (doorways) rendered as translucent polygons to illustrate the boundaries between the empty leaves of the BSP.}
